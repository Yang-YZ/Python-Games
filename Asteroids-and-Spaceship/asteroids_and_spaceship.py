...
def draw(canvas):
    ...
    # update and draw rocks and missiles
    ...
    # update the score when a missile hits a rock
...
def group_collide(sprite_group, ship_object):
    collision = False
    for sprite_object in set(sprite_group):
        if sprite_object.collide(ship_object):
            sprite_group.remove(sprite_object)
            collision =  True
            an_explosion = Sprite([(sprite_object.pos[0]+ship_object.pos[0]) / 2,(sprite_object.pos[1]+ship_object.pos[1]) / 2], 
...
def group_group_collide(sprite_group1, sprite_group2):
    count = 0
    for sprite1 in set(sprite_group1):
        if group_collide(sprite_group2, sprite1):
            sprite_group1.discard(sprite1)
            count += 1
    return count
